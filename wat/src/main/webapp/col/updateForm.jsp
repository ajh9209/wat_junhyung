...
<FORM name='frm' method='POST' action='./update.do'>
	<input type="hidden" name="b_no" value="${dto.b_no }"> <input
		type="hidden" name="b_code" value="${dto.b_code }">
	<div class='table'>
		<table>
			<tr>
				<th>번호</th>
			...
			<tr>
				<th>제목</th>
				<td><input type="text" name="b_title" size="20"
					value='${dto.b_title }'></td>
			</tr>
			<tr>
				<th>내용</th>
				<td><%@ include file="../daumOpenEditor/editor_frame.jsp"%>
					<script>
						//'수정하기' 모드일 때, 다음과 같이 데이터를 다음오픈에디터에 대입
						if ('${dto!=null}' == 'true')
							Editor.modify({
								'content' : '${dto.b_content}'
							});
					</script></td>
			</tr>
			<tr>
	...
	<div class='bottom'>
		<input type='submit' value='수정' onclick="return saveContent();"> <input type='button'
			value='목록' onclick="location.href='./read.do?b_no=${dto.b_no}'">
	</div>
</FORM>
